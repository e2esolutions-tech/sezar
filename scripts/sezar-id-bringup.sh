#!/usr/bin/env bash
# scripts/sezar-id-bringup.sh — SEZ-18 reproducer.
#
# Pre-flight + scan + tail for the hardware-bound sezar-id
# backends (PKCS#11 / YubiHSM / smart card). Mirrors the
# scripts/sezar-net-ebpf-bringup.sh shape: the script
...
#
# Usage:
#   scripts/sezar-id-bringup.sh <vendor-pkcs11-lib> [<collector-url>]
# Env:
#   SEZAR_HSM_PIN              # user PIN; required when the
#                              # token requires login
#   SEZAR_ID_PREFLIGHT_ONLY=1  # check only, no build / scan
#   SEZAR_ID_SKIP_BUILD=1      # reuse cached release binary
...
if [[ -n "${SEZAR_HSM_PIN:-}" ]]; then
    ok "SEZAR_HSM_PIN is set (length=${#SEZAR_HSM_PIN})"
else
    warn "SEZAR_HSM_PIN not set; the scan will see only public-only objects"
fi
...
if [[ "${SEZAR_ID_PREFLIGHT_ONLY:-0}" -eq 1 ]]; then
...
BIN="$ROOT/target/release/sezar-id"
if [[ "${SEZAR_ID_SKIP_BUILD:-0}" -eq 1 && -x "$BIN" ]]; then
    step "skipping build (SEZAR_ID_SKIP_BUILD=1); using $BIN"
else
    step "building sezar-id (release, --features pkcs11)…"
...
[[ -n "${SEZAR_HSM_PIN:-}" ]] && ARGS+=( "--pin-env" "SEZAR_HSM_PIN" )
