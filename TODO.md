...
- [x] systemd unit files (SEZ-24). `dist/systemd/` ships
      one unit per binary: `ree0xq-server.service`,
      `ree0xq-net-live.service`, `ree0xq-cert-host-scan.{service
      ,timer}` (daily 03:17 ± 30 m jitter),
      `ree0xq-id-inventory.{service,timer}` (every 6 h). All
      units run as a dedicated `ree0xq` system user, drop
      ...
      `/var/lib/ree0xq*`, `@system-service` syscall filter
...
### `ree0xq-agility` recommendations engine (on top of the V1 agility scanner)
- [x] **V5.0 per-asset replacement (SEZ-19).** `ree0xq-agility
...
- [x] **V5.1 org migration roadmap (SEZ-20).** `ree0xq-agility
...
- [x] **V5.2 TLS-stack compat matrix (SEZ-21).** `ree0xq-
...
- [x] **V5.3 regulator deadline tracker (SEZ-22).** `ree0xq-
...
- [x] Dashboard integration (SEZ-23). `GET /v1/recommendations`
      on ree0xq-server walks the latest-per-asset map and
      runs `ree0xq_agility::recommend::recommend_for` per
...
- [x] **V4.0 offline classifier (SEZ-15).** `ree0xq-id
      ...
      `asset.kind = hsm_slot`.
- [x] **V4.1 PKCS#11 backend (SEZ-16).** `ree0xq-id
      ...
      plus the
      [`scripts/ree0xq-id-bringup.sh`](scripts/ree0xq-id-bringup.sh)
      pre-flight reproducer.
- [x] **V4.2 AWS KMS backend (SEZ-17).** `ree0xq-id
      ...
      operator-side runbook in
      [`docs/ree0xq-id-aws-kms.md`](docs/ree0xq-id-aws-kms.md).
...
- [x] **V4.3 YubiHSM 2 + smart-card runbook (SEZ-18).**
      ...
      [`docs/ree0xq-id-yubihsm.md`](docs/ree0xq-id-yubihsm.md).
      `scripts/ree0xq-id-bringup.sh` is the host-side
...
### `ree0xq-chain` — offline address-list classifiers
- [x] **V3.0 Bitcoin (SEZ-12).** `ree0xq-chain bitcoin-scan
      ...
      bitcoin:<addr>`.
- [x] **V3.1 Ethereum (SEZ-13).** `ree0xq-chain
      ...
      live-RPC backend.
- [x] **V3.2 QRL (SEZ-14).** `ree0xq-chain qrl-scan
...
### `ree0xq-cert` host-filesystem scanner
- [x] **V2.0 host-scan (SEZ-9).** `ree0xq-cert host-scan
      ...
      to a ree0xq-server; without it, NDJSON to stdout.
...
### Still open under V2
- [x] **V2.1 CT-log scan (SEZ-10).** `ree0xq-cert ct-scan
      ...
      scanner loop.
- [x] **V2.2 internal-CA scan (SEZ-11).** `ree0xq-cert
      ...
      [`docs/ree0xq-cert-vault.md`](docs/ree0xq-cert-vault.md).

---

## V1 critical path

### `ree0xq-core` — event schema
- [x] `crypto_inventory_event` schema v1 (commit `affd080`).
- [x] ts-rs + JsonSchema codegen wired into `cargo test --features
      ts-types` (regenerates `bindings/`, `web/src/types/ree0xq.ts`).
...
- [x] Unit tests `worked_example_alpha_q_matches_paper` and
      ...
      `crates/ree0xq-server/src/posture.rs` — assert the
      implementation reproduces the paper §3.1 numerics within
      ±0.01.
- [x] `crates/ree0xq-server/tests/http_smoke.rs` —
...
- [x] Postgres persistence (SEZ-2). `--database-url` /
      `REE0XQ_DATABASE_URL` flips the in-process `EventStore`
      ...
      brings up postgres + ree0xq-server with the collector
      pointing at `postgres://ree0xq:ree0xq@postgres:5432/ree0xq`.
      Three integration tests in
      `crates/ree0xq-server/tests/pg_smoke.rs` exercise the
...
- [x] mTLS bootstrap (SEZ-6). All four acceptance criteria
      ...
      - Internal CA generated on first boot, persisted to disk
        (`/var/lib/ree0xq/ca/{crt,key}`, key at mode 0600),
      ...
      - `POST /v1/admin/bootstrap-tokens` — admin-gated by
        ...
        `REE0XQ_ADMIN_TOKEN`), returns a single-use UUID token
      ...
      - TLS termination via `--tls`: mints a CA-signed server
        ...
        see the request.
      - Agent-side disk-backed spool (`crates/ree0xq-net/src/
        ...
        (`crates/ree0xq-net/tests/spool_smoke.rs`) exercise the
...
### `ree0xq-net` — network observer (V1 critical path)
- [x] Phase 2.0 pcap-file replay landed (`0d255e9`): `ree0xq-net
...
- [x] Phase 2.2 libpcap live-interface capture: `ree0xq-net live
...
- [x] End-to-end smoke from pcap-file source. New integration test
      `crates/ree0xq-net/tests/end_to_end_smoke.rs` spins
      `ree0xq-server` in-process on an ephemeral port, runs
...
- [ ] `ree0xq-net pq-probe` CLI binary that wraps the Tranco-1k
...
- [x] Per-event scoring with FIPS 203/204/205 awareness, ECDSA
      ...
      `crates/ree0xq-server/src/posture.rs::q_for_event`.
...
- [x] In-process end-to-end test
      `crates/ree0xq-net/tests/end_to_end_smoke.rs` — runs
...
- [x] Shell-level acceptance smoke against the release binaries.
      ...
      `ree0xq-server` on `127.0.0.1:8190`, seeds the canonical 5
      events (3 via `ree0xq-net from-zgrab`, 1 via `ree0xq-net
...
- [x] Docker Compose single-host install — multi-stage `Dockerfile`
      compiles `ree0xq-server` in release and ships a slim Debian
      runtime under a non-root `ree0xq` user with `tini` as PID 1
      and a `curl /healthz` HEALTHCHECK; `compose.yaml` exposes the
      collector on `127.0.0.1:8090` (overridable via
      `REE0XQ_HOST_PORT`). `docker compose up -d` is the V1
