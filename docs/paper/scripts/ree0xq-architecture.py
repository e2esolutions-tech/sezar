#!/usr/bin/env python3
"""
Generate figures/ree0xq-architecture.pdf — Figure 3 of the paper.

Architectural diagram showing the five agents emitting crypto_inventory_event
records into ree0xq-server, with the shared ree0xq-core rollup library and
the React dashboard.
...
def main():
    ...
    # ---------- Agents row (top) ----------
    agents = [
        ("ree0xq-net",    "eBPF: TLS / SSH / IPsec",     1.00, "V1"),
        ("ree0xq-qkd",    "ETSI GS QKD 014 REST",        3.50, "V1"),
        ("ree0xq-agility","Semgrep over source/pkg",     6.00, "V1"),
        ("ree0xq-cert",   "CT logs + host scan",         8.50, "V2"),
        ("ree0xq-chain",  "Public-chain RPC",            11.00, "V3"),
        ("ree0xq-id",     "HSM/KMS/Smart-card",          13.50, "V4"),
    ]
    ...
    # show that every agent depends on ree0xq-core
    for ax_x, ay in agent_positions:
    ...
    # ---------- Title / version note ----------
    ax.text(8, 10.6, "ree0xQ — Reference Architecture (v1.1 event schema)",
    ...
    ax.text(0.3, 0.5,
            ...
            "V1 ships ree0xq-net, ree0xq-qkd, ree0xq-agility, ree0xq-core, ree0xq-server.\n"
            "V2 adds ree0xq-cert; V3 adds ree0xq-chain; V4 adds ree0xq-id.\n"
            "All agents emit the same event shape into the same collector.",
            fontsize=8, color=PHASE_NOTE, style="italic", va="bottom")

    plt.tight_layout()
    plt.savefig("figures/ree0xq-architecture.pdf", bbox_inches="tight")
    plt.savefig("figures/ree0xq-architecture.png", dpi=200, bbox_inches="tight")
    print("Wrote figures/ree0xq-architecture.pdf and .png")
