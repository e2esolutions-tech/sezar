# ree0xq-id V4.2 — AWS KMS bring-up

`ree0xq-id aws-kms-scan` lists every customer-managed key
...
cargo build --release -p ree0xq-id --features aws-kms
...
# 3. Scan.
./target/release/ree0xq-id aws-kms-scan \
    --region us-east-1 \
...
```
[ree0xq_id] aws-kms-scan starting backend="aws-kms"
[ree0xq_id] aws-kms-scan complete keys_seen=12 events_emitted=12
```
...
the SDK chain works outside ree0xq-id.
