# ree0xq-net Phase 2.1 — eBPF TC classifier bring-up

Phase 2.1 is the kernel-side path for the `ree0xq-net` agent. A
...
[p20]: ../crates/ree0xq-net/src/live.rs
[p22]: ../crates/ree0xq-net/src/live.rs

This document is the operator runbook for bringing the path up
on a real host. The whole sequence is also wrapped in
[`scripts/ree0xq-net-ebpf-bringup.sh`](../scripts/ree0xq-net-ebpf-bringup.sh);
...
The `ree0xq-net-ebpf` crate is **deliberately outside** the
...
```
   Compiling ree0xq-net-ebpf v0.1.0-dev (.../crates/ree0xq-net-ebpf)
    Finished `release` profile [optimized] target(s) in ...
```

and the linked object lands at
`target/bpfel-unknown-none/release/ree0xq-net-ebpf` (note: this
...
`ree0xq-net live-ebpf` subcommand.

## Attach + emit events

```bash
sudo ./target/release/ree0xq-net live-ebpf \
    --iface lo \
    --ebpf-object crates/ree0xq-net-ebpf/target/bpfel-unknown-none/release/ree0xq-net-ebpf
```
...
documented in `crates/ree0xq-net/src/spool.rs`.
...
The `ree0xq-net live-ebpf` stdout should produce one or more
...
1. **One agent on one host, 10 minutes of mixed HTTP/HTTPS
   ...
   `pidstat -p $(pgrep ree0xq-net) 5`. Both numbers are
...
2. **Compatibility matrix.** Sanity-check the parser against
   ...
   `crates/ree0xq-net/src/algos.rs` is the single source of
...
3. **Posture scores match the canonical fixtures.** Already
   ...
   `crates/ree0xq-server/src/posture.rs`. The eBPF path emits
...
Run `rustup show` from inside `crates/ree0xq-net-ebpf/`. The
...
Bump `RING_BUFFER_PAGES` in `ree0xq-net-ebpf/src/main.rs` and
...
`sudo tc filter show dev lo ingress` — the `ree0xq_net_tc`
...
the `ree0xq-net live-ebpf` CLI subcommand puts a one-command
entry point on the path. `scripts/ree0xq-net-ebpf-bringup.sh`
