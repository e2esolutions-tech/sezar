# ree0xq-id V4.1 — PKCS#11 backend bring-up

`ree0xq-id pkcs11-scan` opens a vendor PKCS#11 library, walks
...
softhsm2-util --init-token --free --label ree0xq-test --pin 1234 --so-pin 5678
```

## Bring-up

```bash
# 1. Build ree0xq-id with the PKCS#11 feature.
...
pkcs11-tool --module "$LIB" --login --pin 1234 --keypairgen \
    --key-type RSA:4096 --label ree0xq-rsa-test
pkcs11-tool --module "$LIB" --login --pin 1234 --keypairgen \
    --key-type EC:secp256r1 --label ree0xq-ec-test

# 4. Run the scanner. The pin lives in an env var so it
#    doesn't show up in `ps`.
export REE0XQ_HSM_PIN=1234
./target/release/ree0xq-id pkcs11-scan \
    --library "$LIB" \
    --pin-env REE0XQ_HSM_PIN \
...
```
[ree0xq_id] pkcs11-scan starting library="…/libsofthsm2.so" slot=None pin=true
[ree0xq_id] pkcs11-scan complete slots_seen=1 objects_seen=2 events_emitted=2
```
...
- Use a session-scoped PKCS#11 user PIN rather than the SO
  ...
  them, which matches ree0xq-id's read-only contract.
- Run ree0xq-id under systemd with
...
- The scanner is idempotent: re-runs on the same HSM
  ...
  latest-event map in ree0xq-server deduplicates correctly.
...
`pkcs11-tool --module $LIB --login --pin "$REE0XQ_HSM_PIN"
--list-objects` to isolate.

**Empty inventory but keys exist.** ree0xq-id walks
