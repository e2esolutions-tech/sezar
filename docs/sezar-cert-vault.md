# sezar-cert V2.2 — Vault PKI bring-up

`sezar-cert vault-scan` lists every active cert under a
...
- The sezar-cert release binary built (`cargo build --release
  -p sezar-cert`).
- The same `sezar-server` you point other agents at — the
...
vault server -dev -dev-root-token-id=sezar-dev-root &
export VAULT_ADDR=http://127.0.0.1:8200
export VAULT_TOKEN=sezar-dev-root
...
#    --token-env name).
./target/release/sezar-cert vault-scan \
...
- Replace `VAULT_TOKEN=sezar-dev-root` with an AppRole or a
