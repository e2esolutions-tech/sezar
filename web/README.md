...
`sezar-server` on http://127.0.0.1:8090/.
...
`dist/` is a static bundle; in production we expect `sezar-server` to
serve it (the binary picks up a `--web-dir` flag in a later milestone).

## TypeScript schema sync

The wire types in [`src/types/sezar.ts`](./src/types/sezar.ts) are
hand-mirrored from `sezar-core`'s `CryptoInventoryEvent` v1.1 schema.
When `ts-rs` integration lands (the `ts-types` feature on
`sezar-core`), this file will be auto-generated. Until then, any
...
emulator, the sezar-qkd collector, sezar-server, runs sezar-net once
