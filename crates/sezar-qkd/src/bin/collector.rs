//! `sezar-qkd` — ETSI GS QKD 014 collector.
//!
...
//! (typically `sezar-server`). Without a downstream URL the events
//! are logged via `tracing` for local inspection.

use std::time::Duration;

use clap::Parser;
use sezar_qkd::collector::{run, CollectorConfig};
use tracing_subscriber::EnvFilter;

#[derive(Parser, Debug)]
#[command(name = "sezar-qkd", author, version, about)]
struct Args {
    ...
    #[arg(long, default_value = "SAE-SEZAR-COLLECTOR")]
    slave_sae_id: String,
