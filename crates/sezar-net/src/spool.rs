...
//!
//! Failure model: a `sezar-net` agent runs with `--collector
...
//! events.
//!
//! Concurrency: single-writer. Two `sezar-net` processes
...
const SPOOL_FILE: &str = "sezar-net.spool.ndjson";
...
mod tests {
    use super::*;
    use sezar_core::{Asset, AssetKind, Posture, Primitive, PrimitiveRole, SCHEMA_MINOR, SCHEMA_VERSION};
    ...
    fn corrupt_line_is_dropped_not_retained() {
        ...
        std::fs::OpenOptions::new()
            .append(true)
            .open(d.path().join("sezar-net.spool.ndjson"))
            .unwrap()
