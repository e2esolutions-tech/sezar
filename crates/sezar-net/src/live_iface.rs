...
//!
//! Userspace loader for the kernel-side `sezar-net-ebpf` TC
...
//! # Why feature-gated
//!
//! The default build of sezar-net produces a single binary that
...
//!   `target/bpfel-unknown-none/release/sezar-net-ebpf` (built from
//!   the sibling `sezar-net-ebpf` crate with a nightly toolchain
//!   and `bpf-linker`).
...
/// Matches the kernel-side struct layout in `sezar-net-ebpf::main`.
#[repr(C)]
    ...
    F: FnMut(CryptoInventoryEvent),
{
    ...
    let program: &mut SchedClassifier = ebpf
        .program_mut("sezar_net_tc")
        .ok_or_else(|| anyhow::anyhow!("eBPF program `sezar_net_tc` not in object"))?
        .try_into()?;
    program.load()?;
