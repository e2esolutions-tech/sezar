# ree0xq-net-ebpf — Phase 2.1 kernel-side eBPF program
...
[`ree0xq-net`](../ree0xq-net) to parse.
...
the parent `ree0xq-net` crate; see
`crates/ree0xq-net/src/live_iface.rs` and `crates/ree0xq-net`'s
`README.md` for the runtime side. Briefly:

```bash
# From repo root, after building the eBPF object:
cargo build -p ree0xq-net --features live-interface
sudo ./target/debug/ree0xq-net live-iface --interface eth0 \
    --ebpf-object target/bpfel-unknown-none/release/ree0xq-net-ebpf
```
...
(`crates/ree0xq-net/src/live_iface.rs`) is wired behind the
`live-interface` feature, and `ree0xq-net live-ebpf` is the CLI
...
[`docs/ree0xq-net-ebpf.md`](../../docs/ree0xq-net-ebpf.md). For a
