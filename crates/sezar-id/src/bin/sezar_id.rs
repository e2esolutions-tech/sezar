...
//! operator-driven (SEZ-18). See `docs/sezar-id-yubihsm.md`
//! and `docs/sezar-id-smartcard.md` for the runbooks and
//! `scripts/sezar-id-bringup.sh` for the host-side
//! reproducer.
...
use sezar_id::inventory;
use tracing::{error, info, warn};
use tracing_subscriber::EnvFilter;

#[derive(Parser, Debug)]
#[command(name = "sezar-id", author, version, about)]
struct Args {
    #[command(subcommand)]
    cmd: Cmd,
}
...
enum Cmd {
    ...
    InventoryScan {
        ...
        /// caveat as sezar-cert and sezar-chain).
        #[arg(long)]
    ...
    Pkcs11Scan {
        ...
        /// without a PIN sezar-id sees only public objects.
        #[arg(long)]
...
/// Shared Sink — same shape as sezar-cert / sezar-chain.
struct Sink {
    collector: Option<String>,
...
impl Sink {
    fn new(collector: Option<String>, _spool_dir: Option<PathBuf>) -> anyhow::Result<Self> {
        ...
        if collector.is_some() && _spool_dir.is_some() {
            warn!("--spool-dir on sezar-id is a no-op in V4.0; sezar-net::spool reuse follows in a later commit");
        }
...
fn run_pkcs11_scan(
    ...
    anyhow::bail!(
        "pkcs11-scan requires the `pkcs11` feature. \
         Rebuild with: cargo build -p sezar-id --features pkcs11 \
         (a vendor PKCS#11 library must be installed; see \
         docs/sezar-id-pkcs11.md for the SoftHSM bring-up)."
    )
}
...
fn run_aws_kms_scan(
    ...
    use sezar_id::aws_kms::{self, KmsBackend};
...
fn run_aws_kms_scan(
    ...
    anyhow::bail!(
        "aws-kms-scan requires the `aws-kms` feature. \
         Rebuild with: cargo build -p sezar-id --features aws-kms \
         (pulls in aws-sdk-kms; see docs/sezar-id-aws-kms.md)."
    )
}
