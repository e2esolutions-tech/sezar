//! End-to-end smoke for the sezar-id offline inventory
//! classifier (SEZ-15) against a live in-process collector.
//! Mirrors the sezar-chain / sezar-cert smoke shape.
...
use sezar_id::inventory::{self, KeyInventory, SlotInventory};
...
async fn inventory_scan_round_trips_through_collector() {
    ...
    for it in items {
        ...
        assert_eq!(it["source_module"], "sezar-id");
    }
