...
//! posture table defined in `sezar_core::rollup` (when that lands)
...
/// `sezar-agility::UNKNOWN_LEVEL_FALLBACK` (score 0.50).
...
pub fn asset_kind_weight(kind: &sezar_core::AssetKind) -> f32 {
    use sezar_core::AssetKind::*;
    match kind {
        X509Cert => 1.0,
        TlsSession => 0.7,
        SshSession => 0.7,
