//! In-process HTTP smoke test for `sezar-server`.
//!
...
async fn full_ingest_query_loop() {
    ...
    let events = vec![
        event(AssetKind::TlsSession, "tls-modern-1", "sezar-net", modern_classical_tls(), None, Some(agile.clone())),
        event(AssetKind::TlsSession, "tls-locked-1", "sezar-net", modern_classical_tls(), None, Some(locked.clone())),
        event(AssetKind::QkdKme, "KME-A", "sezar-qkd", vec![], Some(cp_qkd.clone()), None),
    ];
