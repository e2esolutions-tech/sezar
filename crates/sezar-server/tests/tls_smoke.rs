...
use sezar_server::{ca::Ca, router_bootstrap, router_main, tls, AppState};
...
async fn full_mtls_bootstrap_to_event() {
    ...
    let r = client
        ...
        .header(sezar_server::enrol::ADMIN_HEADER, ADMIN_SECRET)
    ...
    let r = client
        ...
        .header(sezar_server::enrol::BOOTSTRAP_HEADER, &token)
        .json(&serde_json::json!({ "agent_id": "tls-agent-01" }))
        .send()
        .await
        .unwrap();
