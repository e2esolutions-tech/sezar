...
use sezar_server::enrol::{ADMIN_HEADER, BOOTSTRAP_HEADER};
...
async fn issue_and_enrol_happy_path() {
    ...
    assert_eq!(issued["agent_id"], "sezar-net-01");
    ...
    let r = client
        .post(format!("{base}/v1/enrol"))
        .header(BOOTSTRAP_HEADER, &token)
        .json(&serde_json::json!({ "agent_id": "sezar-net-01" }))
        .send()
        .await
        .unwrap();
