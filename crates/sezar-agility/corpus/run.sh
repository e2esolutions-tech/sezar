...
# run `sezar-agility scan` against it, and persist the resulting
...
#
# Requires:
#   - cargo (builds sezar-agility on demand if not on PATH)
#   - git, jq
...
SCANNER_BIN="$(realpath -m "$ROOT/../../target/release/sezar-agility" 2>/dev/null || echo "")"
if [[ ! -x "$SCANNER_BIN" ]]; then
  SCANNER_BIN="$(realpath -m "$ROOT/../../target/debug/sezar-agility" 2>/dev/null || echo "")"
fi
if [[ ! -x "$SCANNER_BIN" ]]; then
  echo "Building sezar-agility (debug)..." >&2
  (cd "$ROOT/../.." && cargo build -p sezar-agility --bin sezar-agility >/dev/null)
  SCANNER_BIN="$(realpath -m "$ROOT/../../target/debug/sezar-agility")"
fi
